...
def key_expansion(key):
    ...
    while len(key_columns) < 44:
        ...
        if len(key_columns) % 4 == 0:
            ...
            temp[0] ^= RCON[len(key_columns) // 4 - 1]
        key_columns.append([a ^ b for a, b in zip(temp, key_columns[-4])])
    return [key_columns[i:i+4] for i in range(0, len(key_columns), 4)]

# AddRoundKey step
def add_round_key(state, round_key):
    return [[a ^ b for a, b in zip(state_row, round_key_row)] for state_row, round_key_row in zip(state, round_key)]
...
def encrypt(plaintext, key):
    ...
    return [b ^ k for b, k in zip(plaintext, key)] 


# Example usage
if __name__ == "__main__":
    # Test vectors from FIPS-197
    key = [0x2b, 0x7e, 0x15, 0x16, 0x28, 0xae, 0xd2, 0xa6, 0xab, 0xf7, 0x4d, 0x4d, 0x0e, 0xa7, 0x12, 0x0e]
