...
def train_one_round(model,
    ...
    history = model.fit(X, Y, epochs=epochs, batch_size=BATCHSIZE,
                        validation_data=(X_val, Y_val), callbacks=callbacks, verbose = True)
...
def train_neural_distinguisher(starting_round, data_generator, model_name, input_size, word_size, log_prefix='./', _epochs=None, _num_samples=None, _num_val_samples=None, bits_per_token=8, custom_model=None):
    """
    Staged training of model_name starting in `starting_round` for a cipher with data generated by `data_generator`.

    Parameters:
        starting_round: Integer in which round to start the neural network training.
        data_generator: Data_generator(number_of_samples, current_round) returns X, Y.
        model_name: Name of the model ('dbitnet', 'gohr', 'gohr_amsgrad', 'lbce').
        input_size: Input size from cipher (e.g., plain_bits or 512 for ChaCha).
        word_size: Word size of the cipher.
        log_prefix: Directory prefix for saving logs and weights.
        _epochs: Custom number of epochs, defaults to EPOCHS.
        _num_samples: Custom number of training samples, defaults to NUM_SAMPLES.
        _num_val_samples: Custom number of validation samples, defaults to NUM_VAL_SAMPLES.
        bits_per_token: Number of bits grouped into one token (e.g., 8 for byte), used for 'lbce'.
        custom_model: Optional custom model to use (e.g., for 'lbce').
    
    Returns:
        best_round, best_val_acc
    """
    # ------------------------------------------------
    ...
    #     else:
    #         model = make_lbecnet(
    #             input_size=2 * input_size,  # Double for ciphertext pairs
    ...
    #             dropout_rate=0.2
    #         )
    #     optimizer = Adam(learning_rate=1e-4, amsgrad=True)  # Consistent with other models
    ...
    while True:
        ...
        # ------------------------------------------------
        # Create data
        print(f"CREATE CIPHER DATA for round {current_round} (training samples={num_samples:.0e}, validation samples={num_val_samples:.0e})...")
